...
- **IR / two-way / G-League (Propuesta D):** `is_on_ir` (reserva de lesionados, no cuenta en tope, botón en Lesionados + modal plantilla llena al recuperar), `is_two_way` (contrato two-way, salario fijo `TWO_WAY_SALARY`, máx 2/equipo, edad ≤23, toggle en oferta FA + rookies 2ª ronda + firma IA), `g_league_assigned` (G-League ligera: desarrollo +1 atributo/7 días cap `potential`, stats procedimentales en `gleague_season_stats`, exclusión de NBA en simulación/quinteto).
- **Pantalla G-League (`GameScreen.GLeague`, en Competiciones):** `GLeagueController` con **4 pestañas**: PLANTILLA (resumen asignados/two-way + tablas de asignados y elegibles), RESULTADOS (solo partidos GL: próximo del filial + últimos días por fecha, filial resaltado), CLASIFICACIÓN (sub-tabs Este/Oeste, corte playoff tras el 8º) y ESTADÍSTICAS (top-10 promedio PTS/REB/AST/ROB/TAP). La lógica de asignar/recuperar está centralizada en `DatabaseManager.SetGLeagueAssignment(player, assigned)` (toggle + sacar de convocatoria activa), usada por Roster y GLeague. El sidebar añade `SubmenuGLeague`; `ScreenManager` tiene fallback por nombre (`gleague`) como `loadingDocument`.
- **G-League como liga completa:** 30 filiales reales (`gleague_teams`, escudos en `Art/Resources/Teams/GLeague/{size}`, una por franquicia) con **11 prospectos cada una** (`gleague_players`, edades 19-23, sembrados 1 vez/slot vía `EnsureGLeagueSeeded`). Calendario: `GLeagueScheduleGenerator` — doble vuelta intra-conferencia, 28 partidos/filial, SOLO días con NBA (nov→mar, pausa All-Star), jornadas Este/Oeste intercaladas (~7/día) por rotación circular; se genera junto al NBA bajo `season.generated == 0`. Partidos: `DashboardController.ProcessGLeagueGame` deriva en `ProcessSingleGame` por `game_type="gleague"`; convocatoria = prospectos + asignados sanos top-12; `GameSimulator.SimulateGame(..., persistToDb:false)` NO escribe player_game_stats ni aplica records/fatiga/lesiones/moral; stats acumuladas por partido en `gleague_season_stats` (`AddGLeagueGameStat`). Excluidos de QuickNews/Achievements/GameResultCache/finanzas. Playoffs: `GLeaguePostSeason.AdvanceIfNeeded` (en la transacción diaria tras transiciones NBA; NUNCA toca `seasons.phase`) → QF(1v8·4v5·2v7·3v6)→SF→CF→Gran Final, **cada serie al mejor de 3 (primero a 2; mejor seed local en G1 y G3, peor en G2; todos los partidos de una serie comparten `series_label`)**, campeón a `gleague_champions` + noticia; red de seguridad `CompletePostSeason` (SeasonSummary) completa el bracket si el bucle diario no llegó a procesarlo. Clasificación pura en memoria (`GLeagueStandings.Compute`). **INVARIANTE: los GameData GL codifican ids de filial `+GAME_TEAM_ID_OFFSET (1000)` (`DecodeGlTeamId`) para que ninguna query NBA que compare team_id crudo pueda confundirlos; prospectos simulan con id `+PROSPECT_ID_OFFSET (500000)`.** El rollover limpia el calendario GL automáticamente (`DELETE FROM games`); los prospectos persisten.
...
- **IR / two-way / G-League (Propuesta D):** `is_on_ir` excluye del tope (`GetRosterCount`); SACAR IR exige plantilla <17; recuperación limpia `is_on_ir` y, si quedan >17, IA libera al peor y el usuario ve el modal "PLANTILLA LLENA" (`ShowPendingIRReleaseModal`, gate `_pendingIRReleaseCount > 0`). `is_two_way`: salario `TWO_WAY_SALARY=578k`, 2 años, máx 2/equipo (`GetTwoWayCount`), edad ≤23 (`IsEligibleForTwoWay`); oferta FA con toggle TWO-WAY (limpia TO/PO), `ProcessMaturedOffers` salta checks de cap si `offer.is_two_way==1`; IA firma two-way si plantilla llena y quedan plazas; rookies 2ª ronda con `is_two_way=1` si <2. `g_league_assigned`: `GLeagueHelper` (asignar/recuperar, `HasEnoughActive` ≥12, desarrollo +1/7 días cap `potential` recalcula `overall`, stats procedimentales en `gleague_season_stats`); gancho semanal `ProcessGLeagueDevelopment` (paso 2, main thread); excluidos de NBA en `GetActivePlayers`/`AutoFixInjuredLineup`/`QuintetoController` (no a slots 0/1)/`MatchDayController`/All-Star. Al ir a FA (expiración, opción rechazada, trim) se limpian `is_two_way`/`is_on_ir`/`g_league_assigned`.
...
8. **`phase` state machine** (preseason→regular→playin→playoff→finished) — drives dashboard, playoffs, awards. **La G-League NO participa: su postseason se gobierna solo por datos (`game_type` gleague/gleague_playoff).**
...
12. **Codificación de ids G-League en `games`** — home/away = id filial + 1000 (`GLeagueHelper.GAME_TEAM_ID_OFFSET`); cualquier query nueva que lea partidos GL debe descodificar con `DecodeGlTeamId`; los prospectos simulan con id +500000 (`PROSPECT_ID_OFFSET`). No "limpies" estos offsets creyendo que son bugs.
