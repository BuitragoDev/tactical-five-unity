...
- `GameMode.ProManager` differences — only worst teams selectable (`SelectTeamController` `GetWorstTeams(5)`) and new-season offers limited to bottom-10 (`NewSeasonController`). Selecting ProManager in the main menu shows a restrictions modal (`MainMenuController.OpenProModal`). All announced harder rules are implemented: **end-of-season firing if the objective is missed** (`DashboardController.OnActionClicked` → `ShowObjectiveFiredModal`), **budget firing after 2 red-warning days** (`CheckBudgetWarning`, threshold 2 vs Manager's 3), and **no NT-MLE on FA offers** (over-cap FA signings limited to the Taxpayer MLE via `GetMaxOfferBreakdown(proManagerOnly:true)`; NT-MLE hard cap never activates). ProManager also allows changing team each season. Objective/rank logic is centralized in `ObjectiveHelper`.
