...
- `GameMode.ProManager` differences — only worst teams selectable (`SelectTeamController` `GetWorstTeams(5)`) and new-season offers limited to bottom-10 (`NewSeasonController`). Selecting ProManager in the main menu shows a restrictions modal (`MainMenuController.OpenProModal`). Implemented: **end-of-season firing if the objective is missed** (`DashboardController.OnActionClicked` → `ShowObjectiveFiredModal`) and **budget firing after 2 red-warning days** (`CheckBudgetWarning`, threshold 2 vs Manager's 3). The **no-NT-MLE cap rule is announced but not yet enforced** (TODO B20). Objective/rank logic is centralized in `ObjectiveHelper`. Also, ProManager allows changing team each season.
