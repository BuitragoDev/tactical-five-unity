...
- **User-side:** `MarketController` builds trade/offer screens; **AI offers to the player** are generated in `DashboardController.GenerateAITradeOffersForPlayer` and answered via `ShowNextPendingTradeOffer`; `ProcessAITransfers` runs every ≥10 game days inside the transfer window (Sep 1 → Feb 8), reduced to **3-5 game days** during deadline week (Feb 1-8, `IsDeadlineWeek`). AI teams with <12 players sign FAs directly, otherwise try AI trades (max 3), plus `ProcessStarFreeAgentSignings`.
- **AI strategy per team (`DashboardController`):** `enum TeamStrategy { Rebuild, Balanced, Contend }` classified by `GetTeamStrategy` (top 4 conference or 2+ stars OVR≥85 → Contend; bottom 4 or young roster without stars → Rebuild) and cached in `BuildTeamStrategyCache` (`_teamStrategyCache`). `ProcessAITransfers` uses per-team cooldowns (`_teamTradeCooldown`, cleared on season change in `OnEnable`): Contend 6 days (0.45 chance), Rebuild 8 (0.40), Balanced 15 (0.25); all 3-5 during deadline week. **Contend** (`TryFindAITrade`): targets upgrades up to OVR 90, adds a future pick when OVR>84, protects young players (age<26, OVR≥82). **Rebuild** fire sale (`TrySellVeteran`): offers veterans age≥30 for young players (age<25) or picks via `TryExecuteTrade`. Offers to the player are strategy-aware: `PickTradeTarget` (Contend chases stars OVR≥83, Rebuild hunts age≤26) and `BuildOfferPackage` (Contend never offers its young of value; Rebuild pays only with players ≥25). `ProcessStarFreeAgentSignings` prioritizes Contend > Balanced > Rebuild; Rebuild never signs OVR≥85 (protects its draft pick).
- **Deadline week (Feb 1-8):** `ProcessAITransfers` cooldown reduced to 3-5 days; contenders (top 4 conference, `IsTeamContender`) offer draft picks more aggressively; trade offer titles prefixed with `[DEADLINE]` via `IsDeadlineWeek()` check in `ShowTradeOfferModal`. Market UI shows a golden countdown banner `⏳ ÚLTIMOS X DÍAS DE MERCADO DE TRASPASOS` (`DeadlineCountdownBar` in `Market.uxml`).
- **Deadline Day (Feb 7):** `OnActionClicked` intercepts the btnAction: if date is Feb 7 and `_deadlineDayModalShown` is false for the season (`_deadlineModalSeasonId`), a "DEADLINE DAY" overlay is shown (*no day is advanced*). Buttons: "IR AL MERCADO" (navigates to Market) / "CERRAR" (dismisses, sets `_deadlineDayModalShown = true`). The modal appears once per season; subsequent btnAction clicks on Feb 7 simulate normally. `DashboardController.ShowDeadlineDayModal`, `IsFeb7OfYearEnd`.
- **`TradeData.trade_type`:** `"trade"`, `"free_agent"`, `"pick_trade"`, `"sign_and_trade"`.
- **Sign-and-trade (S&T) de FA propio (Bird rights) — dos flujos:**
  - *Extender jugador entrante* (equipo destino, ya existente): al recibir un jugador con `contract_years <= 1`, `MarketController` ofrece "Sign & Trade" que extiende su contrato (+5%) y pone al receptor bajo hard cap del 1er apron.
  - *Firmar y traspasar FA propio* (equipo origen, NBA clásico, nuevo): `MarketController` muestra en el panel de traspaso una sección "FA RECIENTES (BIRD RIGHTS) — SIGN & TRADE"; el manager selecciona a un `IsOwnRecentFA` (salario = max Bird vía `GetMaxOfferBreakdown(isFromSameTeam:true)`), y `ProcessSATrade` lo firma, lo traspasa de inmediato a otro equipo (con jugadores/picks), registra dos `TradeData` (`free_agent` + `sign_and_trade`) y pone al **receptor** bajo hard cap del 1er apron. `TradeHelper.ValidateTrade`/`EvaluateTrade` aceptan `teamASignSalaries`/`teamBSignSalaries` para valorar el nuevo salario firmado (y no descontar roster/nómina del equipo que firma).
  - **IA:** `GenerateAITradeOffersForPlayer` puede proponer S&T por el FA propio del usuario; `ShowNextPendingTradeOffer` lo reconoce (wanted id que no está en roster pero es `IsOwnRecentFA`) y al Aceptar lo firma y traspasa. `ProcessStarFreeAgentSignings`/`TrySignFreeAgent` respetan `pendingSATIds` para no robar un FA con S&T pendiente del usuario.
...
- **Contract options:** renewal and FA offers (`RosterController.SendOffer`, `MarketController.SendFAOffer`) expose **team/player option toggles** (mutually exclusive). If an option is set → `guaranteed_years = max(0, offer_years − 1)`, `has_team_option`/`has_player_option` = 1. On resolution, `ProcessMaturedOffers` persists those fields on the player; inbox messages and the summary modal format them (`FormatContractYears` → `N años (Team Option)` / `M años + Player Option`).
- **Re-firma de FA propio (Bird rights):** `players.last_team_id` guarda el último equipo (set en seed/firma; se conserva al declinar opciones o vencer contrato). `IsOwnRecentFA(p, teamId)` = `team_id==0 && last_team_id==teamId && seasons_with_team>0`. `NewSeasonController` ofrece un modal de re-firma para FA propios que declinaron su player option (oferta diferida `offer_type=0`, madura 7 días); al madurar, `ProcessMaturedOffers` reasigna `team_id` al jugador, o **cancela** la re-firma si mientras maduraba firmó por otro equipo. La firma de FA propios en el Market (`OnSignPlayer`) también usa `isOwnRecentFA` para conservar Bird rights (`isFromSameTeam=true`). La IA (`ProcessStarFreeAgentSignings`/`TrySignFreeAgent`) solo evita ofertas `offer_type==1`, por lo que un re-sign del usuario no es "robado" por la IA.
...
- `ProManager` harder-cap differences — **fully implemented**: objective-based firing at season end (`DashboardController.OnActionClicked` → `ShowObjectiveFiredModal`, ProManager only), earlier budget firing (`CheckBudgetWarning` threshold 2 vs 3), and **no NT-MLE** (over-cap FA signed via Taxpayer MLE; no NT-MLE hard-cap activation). Other differences: worst-team selection + bottom-10 new-season offers + annual team change. Logic centralized in `ObjectiveHelper`. 
