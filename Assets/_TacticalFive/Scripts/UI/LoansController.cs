...
using System.Linq;
    public class LoansController : UIScreenController
{
    protected override GameScreen ScreenId => GameScreen.Loans;
    ...
    void BuildActiveLoanDisplay(VisualElement slot, LoanData loan)
    {
        ...
        if (CursorManager.Instance != null)
            CursorManager.Instance.RegisterHandCursor(payBtn);
    ...
    void BuildLoanEditor(VisualElement slot, int slotIndex)
    {
        ...
        if (CursorManager.Instance != null)
            CursorManager.Instance.RegisterHandCursor(contractBtn);
